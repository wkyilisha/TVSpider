...
public class Proxy extends Spider {
    ...
    private static String buildLogPage() {
        return "<!DOCTYPE html><html><head><meta charset='utf-8'>" +
                ...
                "<script>let last='';setInterval(()=>{fetch('" + LOCAL_HOST + port + "/proxy?do=getlog').then(r=>r.text()).then(d=>{if(d!==last){document.getElementById('logs').innerHTML=d;last=d;window.scrollTo(0,document.body.scrollHeight);}});},800);</script></body></html>";
    }
    ...
    private String rewriteTsToProxy(String content, String baseUrl) {
        ...
        for (String line : content.split("\n")) {
            ...
            if (line.contains(".ts") || line.matches(".*\\.ts\\?.*")) {
                ...
                String proxyTs = Proxy.getUrl() + "?do=stream&url=" + URLEncoder.encode(fullTs, "UTF-8");
